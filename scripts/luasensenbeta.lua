...
return (function(tbl)
    ...
    menu = {
        ["anti aimbot"] = {
            ...
            ["features"] = {
                ...
                safety = ui.new_multiselect("aa", "anti-aimbot angles", prefix("safety"), {"zeus", "knife", "secondary swap on target"}),
                conditions = ui.new_multiselect("aa", "anti-aimbot angles", "\nconditions", {"target has knife", "target has zeus"}),
                weapon = ui.new_combobox("aa", "anti-aimbot angles", "\nweapon", {"pull pistol", "pull zeus (pistol backup)"}),
                secondary_swap_distance = ui.new_slider("aa", "anti-aimbot angles", "\ndistance\n", 200, 700, 350),
        ...
        ["visuals & misc"] = {
            ...
            ["visuals"] = {
                watermark = ui.new_combobox("aa", "anti-aimbot angles", prefix("watermark"), {"-", "bottom", "right", "left"}),
            ...
            ["misc"] = {
                features = ui.new_multiselect("aa", "anti-aimbot angles", prefix("features"), {"fix hideshot", "fast ladder", "animations"}),
                animation = ui.new_multiselect("aa", "anti-aimbot angles", "\nanimation", {"moonwalk", "0 pitch on land"}),
                state = ui.new_combobox("aa", "anti-aimbot angles", "\nstate", {"-", "stand", "air", "duck"}),
                stand_anim = ui.new_slider("aa", "anti-aimbot angles", "\nstand anim", "off", 10, 0, true, "", 1 , {[0] = "off"}),
                air_anim = ui.new_slider("aa", "anti-aimbot angles", "\nair anim", "off", 10, 0, true, "", 1 , {[0] = "off"}),
                duck_anim = ui.new_slider("aa", "anti-aimbot angles", "\nduck anim\n", "off", 10, 0, true, "", 1 , {[0] = "off"}),
                autobuy = ui.new_combobox("aa", "anti-aimbot angles", prefix("auto buy"), {"-", "awp", "scout"})
            }
        },
        ["config"] = {
                ...
                for i, v in next, menu["anti aimbot"] do
                    if i == "submenu" then
                        ...
                        for index, value in next, v do
                            local fixer = true
                            if i == "keybinds" then
                                if index == "left" or index == "right" or index == "forward" or index == "backward" or index == "edge" or index == "teleport" or index == "freestand" then
                                    fixer = false
                                end
                            end
                            if fixer then
    ...
    tbl.refs = {
        aimbot = tbl.ref("rage", "aimbot", "enabled"),
...
---@diagnostic disable-next-line: unbalanced-assignments
    ...
    local function check_charge()
        ...
        local wanted = -14 + (ui.get(ui.reference("rage", "aimbot", "double tap fake lag limit")) - 1) + 3

        dt_charged = shift <= wanted
    end
    z.dtshift = function()
        if not ui.get(tbl.refs.dt[2]) or not ui.get(tbl.refs.dt[1]) then
    ...
    client.set_event_callback("setup_command", z.dtshift)
	...
	tbl.callbacks = {
        ...
        ["command"] = function(arg)
            ...
            if ((tbl.contains(ui.get(menu["anti aimbot"]["features"]["safety"]), "knife") and entity.get_classname(myweapon) == "CKnife") or (tbl.contains(ui.get(menu["anti aimbot"]["features"]["safety"]), "zeus") and entity.get_classname(myweapon) == "CWeaponTaser")) and state == "air duck" and enemy ~= nil then
                for _ = 1,9 do
            ...
            if tbl.contains(ui.get(menu["anti aimbot"]["features"]["safety"]), "secondary swap on target") and enemy ~= nil then
                local secondary_swap_distance = ui.get(menu["anti aimbot"]["features"]["secondary_swap_distance"])
                local secondary_swap_weapon =  ui.get(menu["anti aimbot"]["features"]["weapon"])
                local knife = tbl.contains(ui.get(menu["anti aimbot"]["features"]["conditions"]), "target has knife")
                local zeus = tbl.contains(ui.get(menu["anti aimbot"]["features"]["conditions"]), "target has zeus")
                local local_origin = vector(entity.get_origin(myself))
                ...
                for _, weapon in pairs(get_player_weapons(myself)) do
                    if entity.get_classname(weapon) == "CWeaponTaser" then
                        has_taser = true
                        break
                    end
                end
                if (knife and has_weapon("CKnife")) or (zeus and has_weapon("CWeaponTaser")) then
                    if secondary_swap_weapon == "pull zeus (pistol backup)" and entity.get_classname(myweapon) ~= "CWeaponTaser" then
            ...
            if tbl.contains(ui.get(menu["visuals & misc"]["misc"]["features"]), "fast ladder") and myself ~= nil then
                local pitch, yaw = client.camera_angles()
                if entity.get_prop(myself, "m_MoveType") == 9 then
                    arg.yaw = math.floor(arg.yaw + 0.5)
                    arg.roll = 0
            
                    if arg.forwardmove > 0 then
                        if pitch < 45 then
                            arg.pitch = 89
                            arg.in_moveright = 1
                            arg.in_moveleft = 0
        ...
        ["reset"] = function()
            ...
            if ui.get(menu["visuals & misc"]["misc"]["autobuy"]) ~= "-" then
                ...
                for index, value in next, v do
                    if i == "anti aimbot" and index ~= "submenu" then
                        for ii, vv in next, value do
                            ...
                            if index == "features" then
                                ...
                                if ii == "conditions" or ii == "weapon" or ii == "secondary_swap_distance" then
                                    fix = tbl.contains(ui.get(value["safety"]), "secondary swap on target")
                                end
                            end
                    ...
                    elseif i == "visuals & misc" and index ~= "submenu" then
                        for ii, vv in next, value do
                            fix = true
                            if index == "misc" then
                                if ii == "animation" then
                                    fix = tbl.contains(ui.get(value["features"]), "animations")
                                end
                            end
                            if index == "misc" then
                                if ii == "state" then
                            ...
                            if index == "misc" then
                                if ii == "stand_anim" then
                                    fix = ui.get(value["state"]) == "stand" and tbl.contains(ui.get(value["features"]), "animations")
                                end
                            end
                            if index == "misc" then
                                if ii == "air_anim" then
                                    fix = ui.get(value["state"]) == "air" and tbl.contains(ui.get(value["features"]), "animations")
                                end
                            end
                            if index == "misc" then
                                if ii == "duck_anim" then
                                    fix = ui.get(value["state"]) == "duck" and tbl.contains(ui.get(value["features"]), "animations")
                                end
                            end
        ...
        ["animations"] = function()
			local myself = ent.get_local_player()
            local lp = entity.get_local_player()
            if tbl.contains(ui.get(menu["visuals & misc"]["misc"]["features"]), "animations") and myself ~= nil then
                ...
                local anim_overlay = myself:get_anim_overlay(6)
                local anim_state = myself:get_anim_state()
                local flags = entity.get_prop(lp, "m_fFlags")
                local air = bit.band(flags, 1) == 0
                local crouch = bit.band(flags, bit.lshift(1, 1)) > 0.9
                local air_crouch = bit.band(flags, 1) == 0 and bit.band(flags, bit.lshift(1, 1)) > 0.9
                if tbl.contains(ui.get(menu["visuals & misc"]["misc"]["animation"]), "moonwalk") then
                    ui.set(legs, "never slide")
                    local velocity = entity.get_prop(lp, "m_vecVelocity[0]")
                    if math.abs(velocity) >= 3 then
                        anim_overlay.weight = 1
                        entity.set_prop(lp, "m_flPoseParameter", 0, 7)
                    end
                    entity.set_prop(lp, "m_flPoseParameter", 0, 7)
                end
                if tbl.contains(ui.get(menu["visuals & misc"]["misc"]["animation"]), "0 pitch on land") and (not air) and anim_state.hit_in_ground_animation then
                    entity.set_prop(lp, "m_flPoseParameter", 0.5, 12)
                end
                local function anim(value)
                    local animlayer = myself:get_anim_overlay(1)
                    if value == 1 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 204
                    end
                    if value == 2 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 232
                    end
                    if value == 3 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 227
                    end
                    if value == 4 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 163
                    end
                    if value == 5 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 100
                    end
                    if value == 6 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 10
                    end
                    if value == 7 then
                        animlayer.cycle = 0.12
                        animlayer.weight = 1
                        animlayer.sequence = 232
                    end
                    if value == 8 then
                        animlayer.cycle = 0.42
                        animlayer.weight = 1
                        animlayer.sequence = 232
                    end
                    if value == 9 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 263
                    end
                    if value == 10 then
                        animlayer.cycle = 1
                        animlayer.weight = 1
                        animlayer.sequence = 264
                    end
                end
                anim(ui.get(menu["visuals & misc"]["misc"]["stand_anim"]))
                if air then
                    anim(ui.get(menu["visuals & misc"]["misc"]["air_anim"]))
                end
                if crouch and not air then
                    anim(ui.get(menu["visuals & misc"]["misc"]["duck_anim"]))
                end
            end
        end,
        ["arrows"] = function()
            ...
            if myself ~= nil and entity.is_alive(myself) then
                ...
                if ui.get(menu["visuals & misc"]["visuals"]["watermark"]) == "-" then
                    output = " "
                end
            ...
            if ui.get(menu["anti aimbot"]["keybinds"]["teleport"]) then
                local color = dt_charged and {143, 194, 21} or {255, 0, 0}
                renderer.indicator(color[1], color[2], color[3], 255, "TP")
            end
        end,
		["indicator"] = function()
			local myself = entity.get_local_player()
