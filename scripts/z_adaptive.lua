...
local configs, main, keybinds, labels, indicator, misc,cfgs  = {}, {}, {}, {}, {}, {}, {}
local font = "-> "
local tab = 1
...
local lib = {
    ...
    config_download = function()
        http.get("https://cdn.jsdelivr.net/gh/Inshallah-me/Cloud_GS@refs/heads/main/configs/adaptive.txt"， function(success, response)

            if not success or response.status ~= 200 then
                notify:paint(4, "[Config] Connection Failure")
            end

            local settings = json.parse(response.body)
...
local inti = {
    ...
    main = function()
        main = {
            ...
            tab_2 = ui_new_combobox(TAB[3], TAB[4], prefix(nil, "General").."Tab Selection", "Aimbot", "Other", "Indicators", "Miscellaneous", "Configs"),
        }
    end,
    ...
    config_2 = function()
        ...
        labels.hide_label_other = ui_new_label(TAB[3], TAB[4], "\n")
        labels.other_mod = ui_new_label(TAB[3], TAB[4], prefix(nil, "           Other \aFFFFFFFFModificator"))
    ...
    configs_3 = function()
        ...
        indicator.aimbot_log_tbl = ui_new_multiselect(TAB[3], TAB[4], "\n Aimbot log table", {
            "Hit", "Miss", "Chat"
        })
    ...
    configs_4 = function()
        misc.hide_label_misc = ui_new_label(TAB[3], TAB[4], "\n")
        misc.misc_feature = ui_new_label(TAB[3], TAB[4], prefix(nil, "           Miscellaneous \aFFFFFFFFModificator"))

        misc.air_strafe_fix = ui_new_checkbox(TAB[3], TAB[4], prefix(nil, "Miscellaneous").. "Air strafe fix")
    end,
...
local visible = {
    ...
    labels = function()
        ...
        ui_set_visible(labels.hide_label_other, tab_2)
        ui_set_visible(labels.other_mod, tab_2)
    ...
    indicators = function()
        ...
        local aimbot_log = (ui_get(indicator.aimbot_log)) and ind_tab
        ...
        ui_set_visible(indicator.aimbot_log, ind_tab)
        ui_set_visible(indicator.aimbot_log_tbl, aimbot_log)
    ...
    miscellaneous = function()
        local enable = (ui_get(main.enable))
        local misc_tab = (ui_get(main.tab_2) == "Miscellaneous") and enable

        ui_set_visible(misc.hide_label_misc, misc_tab)
        ui_set_visible(misc.misc_feature, misc_tab)

        ui_set_visible(misc.air_strafe_fix, misc_tab)
    end,
...
local paint = {
    ...
    vanilla = function(color_1, color_2)
        ...
        local ind = {
            dmg_state = lib.contains(ui_get(indicator.feature_tbl), "Min Damage States"),
            hc_state = lib.contains(ui_get(indicator.feature_tbl), "Hitchance States"),
        ...
        local dmg_states = (ind.dmg_state and "D: "..state) or ""

        local hitchance, state = fetch.hitchance(i)
        local hc_states = (ind.hc_state and "H: "..state) or ""

        local Seperate = (ind.dmg_state and ind.hc_state) and " | " or ""
        local print_screen = g_text(color_1, color_2, dmg_states .. Seperate .. hc_states)

        renderer.indicator(255, 255, 255, 255, print_screen)

        local hc_value = ind.hc_value and  "H: " ..((hitchance == 0 and "Off") or hitchance .. "%") or ""
        local dmg_value = ind.dmg_value and "D: " .. (((damage > 100 or damage == 0) and ""..index.index_dmg[damage]) or damage .. "HP") or ""

        local Seperate = (ind.dmg_value and ind.hc_value) and " | " or ""
        local print_screen = g_text(color_1, color_2, dmg_value .. Seperate .. hc_value)
...
local aimbot_log = {
    ...
    --------------------------------------------------------------------------------
    aim_hit = function(e)
        local log_hit = lib.contains(ui_get(indicator.aimbot_log_tbl), "Hit")
        local log_chat = lib.contains(ui_get(indicator.aimbot_log_tbl), "Chat")
        if log_hit then
            ...
            if log_chat then
                chat.print(" \x01[\x01Game\04Sense\x01] " .. "\x01命中\x04 " .. string.lower(target_name) .. "\x01 的 \x04" .. group .. "\x01 造成了 \x04" .. e.damage .. "\x01 伤害 命中率: \x04" .. string.format("%d", e.hit_chance) .. "%")
            end

            client_color_log(255,255,255,"[\0")
            client_color_log(255,255,255," Game\0")
            client_color_log(100,255,100,"Sense\0")
            client_color_log(255,255,255," ]\0")
            client_color_log(255,255,255," 命中 \0")
            client_color_log(0,255,0,string.lower(target_name).."\0")
            client_color_log(255,255,255," 的 \0")
            client_color_log(0,255,0,group.."\0")
            client_color_log(255,255,255," 造成了\0")
            client_color_log(0,255,0,e.damage.."\0")
            client_color_log(255,255,255," 伤害 命中率：\0")
            client_color_log(0,255,0,e.hit_chance)
        end
    end,
    ...
    --------------------------------------------------------------------------------
    aim_miss = function(e)
        local log_miss = lib.contains(ui_get(indicator.aimbot_log_tbl), "Miss")
        local log_chat = lib.contains(ui_get(indicator.aimbot_log_tbl), "Chat")
        if log_miss and e ~= nil then
            local hitgroup_names = { "身体", "头部", "胸部", "肚子", "胳膊", "胳膊", "脚", "腿", "脖子", "?", "gear" }
            local group = hitgroup_names[e.hitgroup + 1] or "?"
            local target_name = entity_get_player_name(e.target)
            local reason
            ...
            if log_chat then
                chat.print("\x01[\x01Game\04Sense\x01] " .. "\x01空了 \x02" .. string.lower(target_name) .. "\x01 的 " .. " \x02" .. group .. "\x01 原因： \x02" .. reason.."\x01 命中率: \x02" .. string.format("%d", e.hit_chance) .. "%")
            end
            ...
            client_color_log(100,255,100,"Sense\0")
            client_color_log(255,255,255," ]\0")
            client_color_log(255,255,255," 空了 \0")
            client_color_log(255,0,0,string.lower(target_name).."\0")
            client_color_log(255,255,255," 的 \0")
            client_color_log(255,0,0,group.."\0")
            client_color_log(255,255,255," 原因：\0")
            client_color_log(255,0,0,reason.."\0")
            client_color_log(255,255,255," 命中率：\0")
            client_color_log(255,0,0,e.hit_chance)
        end
    end,
}
...
local g_callback = {
    ...
    call = function()
        ...
        local paints_debug = function()
            ...
            local get_menu_w, get_menu_h = ui.menu_size()
            local w,h = 200, 250
            local animation = 12
            ...
            ani.debugger_pos[2] = lerp(ani.debugger_pos[2], (menu and get_menu_x + (get_menu_w + 20 - ani.debugger_pos[1])) or (custom and drag_x), animation * globals.frametime())
            ani.debugger_pos[3] = lerp(ani.debugger_pos[3], (menu and get_menu_y + (get_menu_h - h)/2) or (custom and drag_y), animation * globals.frametime())
        ...
        client.set_event_callback("paint_ui", paint_callback)

        local aim_hit_callback = function(e)
            local enable = ui_get(main.enable)
            if not enable then return end
            if indicator.aimbot_log then
                aimbot_log.aim_hit(e)
            end
        end
        ...
        local command_callback = function(cmd)
            ...
            command.air_strafe_fix(cmd)
        end

        client.set_event_callback("setup_command", command_callback)
