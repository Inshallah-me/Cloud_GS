...
local function rectangle_outline(x, y, w, h, r, g, b, a, s)
	s = s or 1
	surface.draw_filled_rect(x, y, w, s, r, g, b, a) -- top
	surface.draw_filled_rect(x, y+h-s, w, s, r, g, b, a) -- bottom
	surface.draw_filled_rect(x, y+s, s, h-s*2, r, g, b, a) -- left
	surface.draw_filled_rect(x+w-s, y+s, s, h-s*2, r, g, b, a) -- right
end
			...
			if visible_alpha > 0 then
				...
				if wx_bot ~= nil then
					...
					if wx_top ~= nil then
						...
						surface.draw_filled_rect(wx_topleft-2, wy_topleft-2, full_width+4, full_height+4, 16, 16, 16, 180*visible_alpha)
						...
						if location_set.distance_width_mp > 0 then
							if width_icon ~= nil then
								-- draw divider
								surface.draw_filled_rect(wx_topleft+width_icon+3, wy_topleft+2, 3, full_height-3, r_m, g_m, b_m, a_m*visible_alpha)
							end
					...
					if location.viewangles_target ~= nil then
						...
						if wx ~= nil then
							...
							if target_size > 1 then
								surface.draw_filled_rect(wx_topleft+target_size-4*dpi_scale, wy_topleft+1, 3, full_height-1, r_m, g_m, b_m, a_m*visible_alpha*location.on_screen_mp)
							end
							...
							if description ~= nil then
								surface.draw_text(wx_topleft + target_size, wy + title_height, math.min(255, r_m*1.2), math.min(255, g_m*1.2), math.min(255, b_m*1.2), a_m*visible_alpha*0.92, old_font, description)
							end
