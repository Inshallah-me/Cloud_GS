...
local references = {
    ...
    dt_fakelag_limit = ui_reference("RAGE", "Aimbot", "Double tap fake lag limit"),
...
local menu_ = {
    ...
    render = {
        alpha = 0,
        list = {"Antiaim", "Visuals", "Misc", "Config"},
        icon = {
...
local function gennerate_antiaim()
    ...
    menu_.Misc = {
        ...
        misc_combobox = ui_new_multiselect(TAB[1], TAB[2], colors.new_blue .. "                    Extra Misc",
            "Resolver", "Teleport", "Enabled Killsay", "Auto Hideshot", "Auto Discharge", "Force DT charging", "Disable force defensive on quickpeek", "Fakelag After DT Shot"),

        resolver_type = ui_new_combobox(TAB[1], TAB[2], "Resolver Version", "Jitter", "Desync"),

        teleport_key = ui_new_hotkey(TAB[1], TAB[2], "Teleport"),
...
local function handle_visible()
    ...
    ui_set_visible(menu_.Misc.teleport_key, misc and contains(ui_get(menu_.Misc.misc_combobox), "Teleport"))
...
local un_safe_func = {
    check_charge = function(self)
        local local_player = entity.get_local_player()
        local m_nTickBase = entity_get_prop(local_player, "m_nTickBase")
        local client_latency = client.latency()
        local shift = math.floor(m_nTickBase - globals_tickcount() - 3 - toticks(client_latency) * .5 + .5 * (client_latency * 10))
    
        local wanted = -14 + (ui_get(references.dt_fakelag_limit) - 1) + 3
    
        un_safe.dt_charged = shift <= wanted
    end,
    weapon_can_fire = function(self, ent)
        if not ui_get(menu_.setup) then
            return
    ...
    unsafe_charging = function(self)
        ...
        local check_charge = function()
            self:check_charge()
        end

        local threat = client.current_threat()

        if not ui_get(references.doubletap[1]) or not ui_get(references.doubletap[2]) or not threat then
            ui_set(references.aimbot, true)

            if un_safe.callback_reg then
                client_unset_event_callback("run_command", check_charge)
                un_safe.callback_reg = false
            end
            return
        end

        if not un_safe.callback_reg then
            client_set_event_callback("run_command", check_charge)
            un_safe.callback_reg = true
        end
        ...
        ui_set(references.aimbot, un_safe.dt_charged)
    end
}
-- end
...
local function exploit_hanlder(cmd)
    ...
    if contains(ui_get(menu_.Misc.misc_combobox), "Teleport") and ui_get(menu_.Misc.teleport_key) and un_safe.dt_charged then
...
local call_backs = {
    ...
    paint_ui = function()
        ...
        aurora()
        manual_indicator()
        if contains(ui_get(menu_.Misc.misc_combobox), "Teleport") and ui_get(menu_.Misc.teleport_key) then
            local color = un_safe.dt_charged and {143, 194, 21} or {255, 0, 0}
